...
Maven and the technologies related to the Maven project are beginning to have a transformation effect on the Java community.
...
On the Figure~\ref{fig:THESIS_FOLDER_STRUCTURE} showed structure of thesis project folder. Project uses standard Maven project folder format.
...
As an open-source library, JUNG provides a common framework for graph/network analysis and visualization. We hope that JUNG will make it easier for those who work with relational data to make use of one another's development efforts, and thus avoid continually re-inventing the wheel."~\cite{JUNG_OVERVIEW}
...
One of the main module in the program is \textsf{Subgraph Extraction} module. Extraction algorithm was explained in the Section~\ref{sec:problem} and in the Appendix~B is pseudo-code, implementation is in the class \textsf{se.lnu.thesis.algorithm.Extractor}.
