...
Additionally to visualization issue it has performance issue -- without any measurement it was seen for an eye that program does not allow smooth interaction.
Low performance issue was in the nature of the JUNG visualization library. It uses very complex hierarchical structure with many utility classes for each visualized object. Flexible architecture has a cost and the cost is high memory consumption.
...
The AWT components are referred to as heavyweight components.''~\cite{JAVA_SWING} More detail information on comparison can be found here.~\cite{AWT_VS_SWING}
